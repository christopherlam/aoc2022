  ...
  (call-with-input-file file
    (lambda (port)
      (let lp ((line (read-line port)) (crates crates-init))
        (cond
         ((eof-object? line) (pk crates))
         (else
          (match (string-split line #\space)
            (("move" (= to-num num) "from" (= to-num from) "to" (= to-num to))
             (let lp1 ((num num))
               (cond
                ((zero? num) (lp (read-line port) crates))
                (else
                 (let ((old-from (vector-ref crates (1- from))))
                   (when (null? old-from)
                     (error "invalid state, old-from empty"))
                   (vector-set! crates (1- to) (cons (car old-from) (vector-ref crates (1- to))))
                   (vector-set! crates (1- from) (cdr old-from))
                   (lp1 (1- num)))))))
            (_ (lp (read-line port) crates)))))))))
